...
connection = MySQLdb.connect(host='localhost', db='group_F', user='', password='')
...
def metabolite():
    cursor.execute("SELECT * FROM Metabolite;")
    results = cursor.fetchall()
    return render_template('Metabolites.html', data=results)
    ...
    with connection.cursor() as cursor:
        ...
        if (results):
            ...
            for row in results:
                ...
                print("<p><strong>description:</strong> %s</p>" % (row[1]))
                print("</div>")  # just
        else:
            print("Query failed")
